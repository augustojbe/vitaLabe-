...
def alterar_dados_exame(request, exame_id):
    ...
    pdf = request.FILE.get('resultado')
    status = request.POST.get('status')
    requer_senha = request.POST.get('requer_senha')
    
    if requer_senha and (not exame.senha):
        messages.add_message(request, constants.ERROR, 'Para exigir a senha, primeiro cri uma')
        return redirect(f'/empresaria/exame_cliente/{exame.id}')
